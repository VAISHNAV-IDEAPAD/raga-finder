...
export default function RagasExplorerTab({ onSelectRagaInFinder }: RagasExplorerTabProps) {
  const [viewMode, setViewMode] = useState<'all' | 'melakarta' | 'popular'>('all');
    ...
    <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto px-4 sm:px-6">
      ...
      <div className="text-center space-y-3">
        <h2 className="text-3xl sm:text-4xl font-black text-stone-900 tracking-tight">
          Explore Classical <span className="text-raga-600">Ragas Library</span>
        </h2>
        <p className="text-sm sm:text-base text-stone-600 max-w-2xl mx-auto">
          Discover the complete 72 Melakarta parent ragas, prominent Janya ragas, arohana-avarohana scales, and audition live swaras with built-in audio synthesis.
        </p>
      </div>
      ...
      <div className="p-4 sm:p-5 rounded-3xl glass-panel shadow-sm border border-amber-200/80 space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          {/* Sub-view switcher */}
          <div className="flex items-center gap-1.5 bg-stone-100/90 p-1 rounded-2xl w-full sm:w-auto">
            <button
              type="button"
        ...
        {(viewMode === 'melakarta' || viewMode === 'all') && (
      ...
      {/* Popular & Seed Ragas Section */}
      {(viewMode === 'popular' || viewMode === 'all') && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredPopular.slice(0, viewMode === 'all' ? 6 : 40).map((r) => {
      ...
      {/* 72 Melakartas Section */}
      {(viewMode === 'melakarta' || viewMode === 'all') && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-bold text-stone-900 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-amber-600" />
              <span>72 Melakarta Scheme (Parent Janaka Ragas)</span>
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-stone-100 text-stone-700 border border-stone-200">
                {filteredMelakartas.length} Ragas
              </span>
            </h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {filteredMelakartas.slice(0, viewMode === 'all' ? 12 : 72).map((m) => {
          ...
          {viewMode === 'all' && filteredMelakartas.length > 12 && (
            <div className="text-center pt-2">
              <button
                type="button"
                onClick={() => setViewMode('melakarta')}
                className="px-5 py-2.5 rounded-xl bg-white border border-amber-300 text-amber-900 text-xs font-bold shadow-xs hover:bg-amber-50 transition-colors inline-flex items-center gap-1.5"
              >
                <span>View all 72 Melakartas</span>
